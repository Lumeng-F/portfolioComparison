...
def get_bench_his(ticker):
    ...
    benchmark_his['Buy Cost'] = pd.DataFrame(trans.groupby('Date', sort=False).sum()['Cost'])
...
def user_df(filepath):
    trans = pd.read_csv(fpath)
